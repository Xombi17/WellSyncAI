...
const familyMembers = [
  { id: '1', name: 'Me', role: 'Primary', image: 'https://picsum.photos/seed/me/100/100' },
  { id: '2', name: 'Aarav', role: 'Child', image: 'https://picsum.photos/seed/aarav/100/100' },
  { id: '3', name: 'Mother', role: 'Elder', image: 'https://picsum.photos/seed/mother/100/100' },
];
    ...
    <section>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {familyMembers.map((member) => (
          <Link href={`/timeline/${member.id}`} key={member.id} className="bg-[#f3f6fd] dark:bg-slate-800 p-6 rounded-[2rem] shadow-[8px_8px_16px_rgba(0,0,0,0.05),-8px_-8px_16px_rgba(255,255,255,0.8)] dark:shadow-[8px_8px_16px_rgba(0,0,0,0.4),-8px_-8px_16px_rgba(255,255,255,0.05)] hover:shadow-[10px_10px_20px_rgba(0,0,0,0.05),-10px_-10px_20px_rgba(255,255,255,0.8),inset_4px_4px_10px_rgba(255,255,255,0.9),inset_-4px_-4px_10px_rgba(0,0,0,0.02)] dark:hover:shadow-[10px_10px_20px_rgba(0,0,0,0.4),-10px_-10px_20px_rgba(255,255,255,0.05),inset_4px_4px_10px_rgba(255,255,255,0.1),inset_-4px_-4px_10px_rgba(0,0,0,0.5)] hover:-translate-y-1 transition-all cursor-pointer text-center block">
            <div className="w-20 h-20 rounded-[1.25rem] bg-white dark:bg-slate-700 p-1 mx-auto mb-4 shadow-[inset_2px_2px_4px_rgba(0,0,0,0.1)] dark:shadow-[inset_2px_2px_4px_rgba(0,0,0,0.5)]">
              <img src={member.image} alt={member.name} className="w-full h-full object-cover rounded-xl" />
            </div>
            <h3 className="font-black text-lg text-slate-800 dark:text-white">{member.name}</h3>
            <p className="text-sm font-bold text-blue-400">{member.role}</p>
          </Link>
        ))}
