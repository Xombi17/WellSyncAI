...
export default function SettingsPage() {
  const { language, setLanguage, voiceEnabled, toggleVoice, notificationsEnabled, toggleNotifications, logout } = useAuthStore();
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-2xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="font-heading font-800 text-2xl text-white mb-1">Settings</h1>
        <p className="text-sm text-white/35 mb-8">Manage your preferences and account</p>
      </motion.div>
      ...
      <div className="space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <p className="font-heading font-700 text-sm text-white">Language Hub</p>
              <p className="text-xs text-white/30">Choose your preferred language</p>
            </div>
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-white/30">Receive health reminders</p>
              </div>
            </div>
            <button onClick={toggleNotifications}
              className={`w-12 h-7 rounded-full transition-colors relative ${notificationsEnabled ? 'bg-teal-500' : 'bg-white/10'}`}>
              <div className={`w-5 h-5 rounded-full bg-white absolute top-1 transition-all ${notificationsEnabled ? 'left-6' : 'left-1'}`} />
            </button>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={toggleVoice}
              ...
              <div className={`w-5 h-5 rounded-full bg-white absolute top-1 transition-all ${voiceEnabled ? 'left-6' : 'left-1'}`} />
            </button>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-white">Export Health Data</p>
                <p className="text-xs text-white/30">Download records for doctor visits</p>
              </div>
            </div>
            <button onClick={handleExport} className="px-4 py-2 bg-white/[0.06] border border-white/[0.08] rounded-xl text-xs font-medium text-white/60 hover:text-teal-400 hover:border-teal-500/20 transition-colors">
              Download JSON
            </button>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
          ...
          <p className="text-xs text-white/30 leading-relaxed">WellSync AI does not provide medical diagnoses or treatment advice. All health schedules are based on verified guidelines. Always consult healthcare professionals for medical decisions.</p>
        </motion.div>

        {/* Logout */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
          <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 py-3.5 bg-coral-500/10 border border-coral-500/20 rounded-2xl text-coral-400 font-heading font-700 text-sm hover:bg-coral-500/20 transition-colors">
